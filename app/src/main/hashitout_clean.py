...
# section 0 :: boot + shared guts
# this file is still one big snake, but now the moving parts are marked out
# so a tired human can stalk bugs without stepping on nine rakes
...
class AnalysisEngine:
    def __init__(self, wordlist: set = None, output_dir: str = './output', max_depth: int = 3, stegopw_wordlist: str = None,
                 verbose: bool = True, flags: dict = None, password: str = None):
        self.wordlist = wordlist or set()
        self.output_dir = output_dir
        self.verbose = verbose
        self.flags = flags or {}
        self.password = password or self.flags.get('password') or ''
        self.max_depth = max_depth
    ...
    def _analyze_zip(self, data: bytes) -> List[Finding]:
        ...
        if self.password:
            findings += _try_password_on_zip(data, self.password, self, 'ZIP')
        return findings



def _openssl_bytes_to_key(password: bytes, salt: bytes, key_len: int, iv_len: int):
    # section 6a :: tiny openssl compat shim
    out = b''
    prev = b''
...
def _try_password_on_openssl_blob(data: bytes, password: str, engine=None, source_label='OPENSSL'):
    findings = []
    if not password or not data.startswith(b'Salted__') or len(data) < 32:
        return findings
    try:
        from Crypto.Cipher import AES
        salt = data[8:16]
        blob = data[16:]
        for key_len, label in ((32, 'AES-256-CBC'), (24, 'AES-192-CBC'), (16, 'AES-128-CBC')):
            try:
                key, iv = _openssl_bytes_to_key(password.encode('utf-8'), salt, key_len, 16)
                plain = AES.new(key, AES.MODE_CBC, iv).decrypt(blob)
                if plain:
                    pad = plain[-1]
                    if 1 <= pad <= 16 and plain.endswith(bytes([pad]) * pad):
                        plain = plain[:-pad]
                score = _chain_score(plain.decode('utf-8', errors='ignore'), getattr(engine, 'wordlist', None) or set())
                if score >= 18:
                    findings.append(Finding(
                        method=f'passworded file decrypt ({label})',
                        result_bytes=plain,
                        confidence='HIGH' if score >= 40 else 'MEDIUM',
                        note=f'user password hit on openssl salted blob using {label.lower()}'
                    ))
                    break
            except Exception:
                continue
...
def _try_password_on_zip(data: bytes, password: str, engine=None, source_label='ZIP'):
    findings = []
    if not password:
        return findings
    try:
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
            for info in zf.infolist():
                if info.is_dir():
                    continue
                try:
                    payload = zf.read(info.filename, pwd=password.encode('utf-8'))
                except Exception:
                    continue
                ft = detect_filetype(payload)
                note = f'user password opened zip member {info.filename}'
                findings.append(Finding(
                    method=f'passworded zip member: {info.filename}',
                ...
                try:
                    ...
                    if txt.strip() and engine:
                        findings.extend(engine.analyze_string(txt[:4096], info.filename))
                except Exception:
                    pass
    except Exception:
        pass
...
def _password_cipher_findings(data: str, password: str, wordlist=None):
    # section 6b :: one user-supplied password, no guessing games
    findings = []
    if not password:
        return findings
    seen = set()

    def _push(method, plain, note=''):
        if not plain:
            return
        plain = plain if isinstance(plain, str) else plain.decode('utf-8', errors='ignore')
        if len(plain.strip()) < 2 or plain in seen:
            return
        seen.add(plain)
        score = _chain_score(plain, wordlist or set())
        if score < 12:
            return
        findings.append(Finding(
            method=method,
            result_text=plain,
            confidence='HIGH' if score >= 40 else ('MEDIUM' if score >= 22 else 'LOW'),
            note=(note or f'user password trial, score {score}')[:240],
        ))

    try:
        if any(c.isalpha() for c in data):
            _push('Vigenère (user password)', decode_vigenere(data, password), f'user key="{password}"')
    except Exception:
        pass
    try:
        raw = data.encode('latin-1', errors='ignore')
        if raw:
            key = password.encode('utf-8', errors='ignore') or b''
            if key:
                xor_plain = bytes(b ^ key[i % len(key)] for i, b in enumerate(raw))
                _push('XOR repeating-key (user password)', xor_plain, 'repeating xor using supplied password bytes')
                _push('RC4 (user password)', decode_rc4(raw, key), 'rc4 using supplied password bytes')
    except Exception:
        pass
    return findings
    ...
    def _try_ciphers(self, data: str) -> List[Finding]:
        ...
        if self.password:
            findings.extend(_password_cipher_findings(data, self.password, self.wordlist))
            ...
            if (4 <= len(_hill_in) <= 30 and " " not in data.strip()
                ...
                for score, key, plain in brute_hill_2x2(data, self.wordlist):
                    findings.append(Finding(method="Hill 2x2 cipher",
                        ...
                        note="hill key=%s" % key))
        if len(data) >= 4:
            layer_depth = int(self.flags.get('layer_depth') or 5)
            for score, chain, plain in beam_chain_decode(
                    data, self.wordlist, max_depth=layer_depth, beam_width=24, min_score=10,
                    show_progress=self.verbose):
                findings.append(Finding(
                    method="Decoded chain: %s" % chain,
                    result_text=plain,
                    confidence="HIGH" if score >= 80 else ("MEDIUM" if score >= 48 else "LOW"),
...
# section 7 :: layered decode graph
# this is the part that sniffs around inside snarled-up text and keeps a leash on chaos
...
def _apply_transform(text, name):
    ...
    def _hexd(t):
        try:
            c = t.replace(' ', '').replace('0x', '').replace('\\x', '').replace(':', '')
    ...
    def _bacon(t, mode):
        try:
            ...
            if mode == '01':
                ...
                s = s.replace('I', 'A').replace('O', 'B')
            bm = {'AAAAA':'A','AAAAB':'B','AAABA':'C','AAABB':'D','AABAA':'E','AABAB':'F','AABBA':'G','AABBB':'H','ABAAA':'I','ABAAB':'J','ABABA':'K','ABABB':'L','ABBAA':'M','ABBAB':'N','ABBBA':'O','ABBBB':'P','BAAAA':'Q','BAAAB':'R','BAABA':'S','BAABB':'T','BABAA':'U','BABAB':'V','BABBA':'W','BABBB':'X','BBAAA':'Y','BBAAB':'Z'}
            cl = re.sub(r'[^AB]', '', s)
            if len(cl) < 5:
                return None
    ...
    def _scytale(t, cols):
        ...
        if n < cols:
            return None
        rows = (n + cols - 1) // cols
        pad = t2.ljust(rows * cols)
        return ''.join(pad[c * rows + r] for r in range(rows) for c in range(cols)).strip()

    def _vigenere_key(t, key):
        return decode_vigenere(t, key)

    static = {
        'reverse': lambda t: t[::-1],
        'atbash': decode_atbash,
        'base64': lambda t: (lambda r: r if len(r) > 2 else None)(_b64.b64decode(t + '==').decode('utf-8', errors='ignore')),
        'base64url': lambda t: (lambda r: r if len(r) > 2 else None)(_b64.urlsafe_b64decode(t + '==').decode('utf-8', errors='ignore')),
        'base32': _b32,
        'hex': _hexd,
        ...
        'morse': decode_morse,
        'binary': lambda t: (''.join(chr(int(tk, 2)) for tk in t.strip().split()) if all(all(c in '01' for c in tk) and len(tk) == 8 for tk in t.strip().split()) else None),
        'a1z26': decode_a1z26,
        'a1z26_reverse': lambda t: (''.join(chr(ord('z') - n + 1) for n in [int(x) for x in re.split(r'[\\s,\-]+', t.strip()) if x.isdigit()] if 1 <= n <= 26) or None),
        'polybius': decode_polybius,
        'tap_code': decode_tap_code,
        'bacon_ab': lambda t: _bacon(t, 'ab'),
        'bacon_01': lambda t: _bacon(t, '01'),
        'bacon_io': lambda t: _bacon(t, 'io'),
        'phone_keypad': lambda t: (''.join({'2':'a','22':'b','222':'c','3':'d','33':'e','333':'f','4':'g','44':'h','444':'i','5':'j','55':'k','555':'l','6':'m','66':'n','666':'o','7':'p','77':'q','777':'r','7777':'s','8':'t','88':'u','888':'v','9':'w','99':'x','999':'y','9999':'z'}.get(tk, '?') for tk in t.strip().split()) or None),
        'nibble_swap': lambda t: (lambda r: r if _pr(r) > 0.8 else None)(''.join(chr(((ord(c) & 0x0F) << 4) | ((ord(c) & 0xF0) >> 4)) for c in t)),
        'bits_reverse': lambda t: (lambda r: r if _pr(r) > 0.8 else None)(''.join(chr(int(f'{ord(c):08b}'[::-1], 2)) for c in t)),
        'mirror_alphabet': decode_atbash,
        'dvorak_to_qwerty': lambda t: ''.join('qwertyuiopasdfghjklzxcvbnm'['pyfgcrlaoeuidhtnsqjkxbmwvz'.index(c)] if c in 'pyfgcrlaoeuidhtnsqjkxbmwvz' else c for c in t.lower()),
        'keyboard_shift': lambda t: ''.join('pyfgcrlaoeuidhtnsqjkxbmwvz'['qwertyuiopasdfghjklzxcvbnm'.index(c)] if c in 'qwertyuiopasdfghjklzxcvbnm' else c for c in t.lower()),
        'leet_speak_decode': decode_leetspeak,
        'decimal_bytes': lambda t: (lambda nums: ''.join(chr(n) for n in nums) if len(nums) >= 2 and all(32 <= n <= 126 for n in nums) else None)([int(x) for x in re.split(r'[\s,]+', t.strip()) if x.isdigit()]),
    }

    try:
        if name in static:
            return static[name](text)
        if name.startswith('rot') and name[3:].isdigit():
            n = int(name[3:])
            return ''.join(chr((ord(c) - 65 + n) % 26 + 65) if c.isupper() else chr((ord(c) - 97 + n) % 26 + 97) if c.islower() else c for c in text)
...
def _looks_plaintext(text: str) -> bool:
    ...
    return pr > 0.93 and ar > 0.45 and 2.4 <= ent <= 5.8 and words >= 2
...
def _family_for_transform(name: str) -> str:
    ...
    if name == 'binary':
        return 'binary'
    if name.startswith('railfence'):
        return 'railfence'
    if name.startswith('scytale'):
        return 'scytale'
    if name.startswith('vigenere_'):
        return 'vigenere'
    if name.startswith('xor_0x'):
        return 'xor'
    if name in ('morse', 'polybius', 'tap_code', 'a1z26', 'a1z26_reverse', 'phone_keypad'):
        return name
    if name.startswith('bacon_'):
        return 'bacon'
    return name


def _beam_seed_candidates(text: str):
    seeds = []
    seen = set()

    def _push(label, value):
        value = (value or '').strip()
        if len(value) < 4 or value in seen:
            return
        seen.add(value)
        seeds.append((label, value))

    _push('full', text)
    for line in re.split(r'\r?\n+', text):
        if len(line.strip()) >= 8:
            _push('line', line)
    compact_alpha = ''.join(c for c in text if c.isalpha())
    compact_alnum = ''.join(c for c in text if c.isalnum())
    _push('alpha-only', compact_alpha)
    _push('alnum-only', compact_alnum)
    ...
    for m in re.finditer(r'\(([^)]+)\)|\[([^\]]+)\]|\{([^}]+)\}', text):
        ...
        if len(frag.strip()) >= 6:
            _push('bracketed', frag)
    return seeds or [('full', text)]
...
def _candidate_transforms(text: str, chain_steps):
    ...
    if re.fullmatch(r'[0-9\s,.-]+', s) and len(s.split()) >= 2:
        transforms.extend(['a1z26', 'a1z26_reverse'])
    if all(c in '.- /|\n\t' for c in s) and len(compact) >= 6:
        transforms.append('morse')
    if re.fullmatch(r'[1-5\s]+', s) and len(compact) >= 10:
        transforms.extend(['polybius', 'tap_code'])
    if re.search(r'\b[2-9]{1,4}(?:\s+[2-9]{1,4})+\b', s):
        transforms.append('phone_keypad')
    transforms.extend(['bacon_ab', 'bacon_01', 'bacon_io', 'leet_speak_decode', 'decimal_bytes', 'nibble_swap', 'bits_reverse'])
    for rails in range(2, 9):
        transforms.append(f'railfence{rails}')
    for cols in range(2, 9):
        transforms.append(f'scytale{cols}')
    alpha_only = ''.join(c for c in s if c.isalpha())
    if len(alpha_only) >= 8:
        transforms.extend([f'vigenere_{k}' for k in ('key', 'secret', 'password', 'crypto', 'flag', 'hashitout')])
...
def _beam_prompt_continue(best_hits, threshold):
    if not sys.stdin.isatty():
        return False
    print('\n  top hits crossed threshold. pause or push deeper?')
    for idx, (score, chain, plain) in enumerate(best_hits[:5], 1):
        print(f'  [{idx}] {score:>3}  {chain}')
        print(f'      {plain[:120].replace(chr(10), " ")}')
    try:
        raw = input(f'  continue past {threshold}%? [y/N] ').strip().lower()
    except Exception:
        return False
    return raw in ('y', 'yes')


def _beam_chain_decode_impl(text, wordlist, max_depth=5, beam_width=24, min_score=16, show_progress=True):
    import sys, time, random
    matrix_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789@#$%^&*<>?/'
    grn = '\033[38;5;82m'; dim = '\033[2m'; rst = '\033[0m'; clr = '\033[2K\r'
    t_start = time.time()
    total_ops = max_depth * max(beam_width, 1) * 24
    op_count = 0
    threshold = int((_HIO_ACTIVE_FLAGS or {}).get('stop_threshold') or 80)
    auto_continue = bool((_HIO_ACTIVE_FLAGS or {}).get('continue_after_threshold'))
    ...
    def _progress(current_transform, chain_steps, hits):
        ...
        noise = ''.join(random.choice(matrix_chars) for _ in range(28))
        chain_str = (' -> '.join(chain_steps) + ' -> ' + current_transform)[-28:]
        sys.stderr.write(f"{clr}  {grn}{noise}{rst}  {dim}depth {len(chain_steps)+1}  chain: {chain_str:<30}  hits: {hits}  ~{eta:.0f}s{rst}")
        sys.stderr.flush()

    candidates = []
    for label, seed in _beam_seed_candidates(text):
        candidates.append((0, [], seed, label))
    found = []
    seen_texts = {_normalize_visible(text)[:800]}
    family_cap = {'rot': 4, 'base64': 4, 'base32': 2, 'hex': 2, 'reverse': 4, 'atbash': 4, 'url': 2, 'html': 2, 'binary': 2, 'a1z26': 2, 'morse': 2, 'railfence': 4, 'scytale': 4, 'vigenere': 4, 'bacon': 3, 'xor': 3}
    user_prompted = False

    for depth in range(1, max_depth + 1):
        next_candidates = []
        fam_counts = {}
        for _, chain, current, origin in candidates:
            ...
            for transform in _candidate_transforms(current, chain):
                ...
                if _beam_should_skip(current, result, chain, transform):
                    continue
                norm = _normalize_visible(result)[:800]
                if norm in seen_texts:
                    continue
                seen_texts.add(norm)
                score = _chain_score(result, wordlist) - _beam_chain_penalty(chain + [transform])
                if _looks_plaintext(result):
                    score += 10
                new_chain = chain + [transform]
                if score >= min_score:
                    found.append((score, ' -> '.join(new_chain), result, origin))
                fam_counts[fam] = fam_counts.get(fam, 0) + 1
                next_candidates.append((score, new_chain, result, origin))
        ...
        if found:
            top_preview = sorted(found, key=lambda x: x[0], reverse=True)[:5]
            if top_preview[0][0] >= threshold and not auto_continue and not user_prompted:
                user_prompted = True
                if not _beam_prompt_continue([(s, c, p) for s, c, p, _ in top_preview], threshold):
                    break

    if show_progress:
        elapsed = time.time() - t_start
        sys.stderr.write(f"{clr}  {grn}chain analysis complete{rst}  {dim}{elapsed:.2f}s  {len(found)} candidates{rst}\n")
        sys.stderr.flush()

    results = []
    seen_out = set()
    for score, chain, result, origin in sorted(found, reverse=True):
        norm = _normalize_visible(result)[:600]
        if norm in seen_out:
            continue
        seen_out.add(norm)
        chain_label = f'{origin} :: {chain}' if origin != 'full' else chain
        results.append((score, chain_label, result))
        if len(results) >= 12:
            break
    return results


def beam_chain_decode(text, wordlist, max_depth=5, beam_width=28, min_score=14, show_progress=True):
    flags = _HIO_ACTIVE_FLAGS
    if flags.get('full_nasty'):
        max_depth = max(max_depth, min(_FULL_NASTY_PROFILE['beam_depth'], 5))
...
def build_parser():
    ...
    p.add_argument('--password', metavar='TEXT', help='single user-supplied password for passworded decrypt attempts')
    p.add_argument('--layer-depth', dest='layer_depth', metavar='N', type=int, default=5, help='max layered decode depth for the chain graph')
    p.add_argument('--continue-after-threshold', dest='continue_after_threshold', action='store_true', help='do not pause when layered decode score crosses the stop threshold')
...
def main():
    ...
    flags['password'] = getattr(args, 'password', '') or ''
    flags['layer_depth'] = getattr(args, 'layer_depth', 5) or 5
    flags['continue_after_threshold'] = bool(getattr(args, 'continue_after_threshold', False))
    flags['stop_threshold'] = 80
