...
object TextScorer {
    ...
    fun score(text: String): Double {
        ...
        val wordHits = EnglishWords.common.sumOf { if (lower.contains(it)) 4.5 else 0.0 }
        val wordBoundaryHits = lower.split(Regex("\\s+|[^a-z0-9]+"))
            .count { it in EnglishWords.common }
            .toDouble() * 2.5
        val freq = frequencySimilarity(normalized) * 22.0
        ...
        return printableRatio * 25 + alphaRatio * 16 + wordHits + wordBoundaryHits + freq + tetras + bonus + punctuationPenalty
    }

    fun confidence(score: Double): Confidence = when {
        score >= 54.0 -> Confidence.HIGH
        score >= 28.0 -> Confidence.MEDIUM
        else -> Confidence.LOW
    }
